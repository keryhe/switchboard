...
## 12. Management API (Phase 4)

`Keryhe.Switchboard.Management` is a class library mapped into the same `WebApplication` as every other endpoint (`MapSwitchboardManagement(app)`, called from `Program.BuildApp` after every other route) — **not** a separate process or listener. It depends on `Core + Protocol` (corrected from the roadmap's original `Core` only — see [06-project-plan.md](06-project-plan.md)'s dependency graph): the health endpoint needs `IHubRegistry` and the send endpoints need hub-protocol writing, both Protocol-level concerns.

**Fails closed by absence, not by 401.** `MapSwitchboardManagement` reads `EnableManagementApi`/`ManagementSigningKey` from raw `IConfiguration` rather than `IOptions<SwitchboardOptions>.Value` — resolving the bound options here, during `Program.BuildApp` and before `StartAsync`, would trigger the *entire* options `.Validate(...)` chain early, moving an unrelated configuration's `OptionsValidationException` (e.g. Pattern A's `TrustedProxyNetworks` check) from `StartAsync` to `BuildApp` merely because this method happened to touch the same options object first. With the feature off or no key configured, `/api/v1/*` routes are never mapped — 404, not 401 — and turning the feature on with no key fails startup validation instead.

**`ManagementAuthFilter`** is an `IEndpointFilter` applied to the whole `/api/v1` route group: it checks `ManagementAllowedNetworks` first (403 if the caller's IP is outside the allowlist, using the same `PeerNetworkMatcher` §1's `TrustedProxyNetworks` check uses), then validates the bearer token via `ITokenService.Validate(token, SwitchboardTokenType.Management)` (401 if missing or invalid). This mirrors every other authenticated endpoint's hand-rolled `Authorization: Bearer` pattern (ADR-004) rather than configuring `AddAuthentication`/`AddJwtBearer` for one route group — the reason `Microsoft.AspNetCore.Authentication.JwtBearer` was removed from `Keryhe.Switchboard.Server`'s dependencies in this phase (it was referenced but never used anywhere in the codebase).

**Group endpoints call `IGroupMembershipService`, never their own copy of the logic.** `PUT`/`DELETE /api/v1/hubs/{hub}/groups/{group}/connections/{id}` do exactly what `RoutingServerEnvelopeDispatcher`'s `add_to_group`/`remove_from_group` envelope handling does — update the registry, then forward to the owning node via `IBackplane` when the connection isn't local — because that forward is the Phase 3 Slice 7 bug fix (§7), and its failure mode is silent: a client joins a group and simply never receives a message from it, with no exception. The body was extracted into `Keryhe.Switchboard.Core.GroupMembershipService` (constructor takes a plain `string nodeId`, not `IOptions<SwitchboardOptions>` — Core has no dependency beyond the BCL) and both the envelope dispatcher and the management endpoint call the same implementation.

**Send endpoints are the service's first payload *producer*.** `POST /api/v1/hubs/{hub}/send` (and the user/group variants) take `{target, arguments}` and must construct real hub-protocol `Invocation` bytes — something the service has otherwise never done outside `ClientFrameWriter`'s three-message charter (handshake response, `Ping`, `Close`). Rather than widening that type, `ManagementInvocationWriter` (`Keryhe.Switchboard.Protocol/Framing/`) is added beside it with its own narrow charter: invocation messages originating from the management REST API only. It maps each JSON argument to a CLR primitive (`string`/`long`/`double`/`bool`/`null`/nested array/nested object) explicitly — critically, with `long` and `double` branched separately rather than `e.TryGetInt64(out var l) ? l : e.GetDouble()`, whose ternary best-common-type is `double` and would silently float every integer — before encoding for both `json` and `messagepack`, since a management send has no way to know in advance which protocol each recipient negotiated (the same reason app-server fan-out carries `Payloads`, §5/§7 D7). Skipping the primitive mapping does not throw: `MessagePackHubProtocol` silently encodes raw `JsonElement` arguments as empty maps, with no exception, while `JsonHubProtocol` happens to handle them fine — the single most likely silent defect in this phase, which is why it is a dedicated regression test (`ManagementSendEndpointsTests`) rather than an incidental assertion.

Management sends bypass app servers and hub code entirely — they inject straight into `IMessageRouter.BroadcastAsync`/`SendToGroupAsync`/`SendToUserAsync`, matching Azure SignalR Service's own REST API semantics.

**Cluster-wide reads go through `IClusterInventory`, never `Keryhe.Switchboard.Orleans` directly.** `GET /api/v1/hubs/{hub}/connections` (mandatorily paginated — `IConnectionRegistry.GetAllAsync`'s own doc comment says it is one grain call per connection) and `GET /api/v1/health` (per-hub server-connection counts, cluster-wide client-connection total) both need to answer questions neither `IHubRegistry.GetAllHubs()` nor `ILocalTransportRegistry.GetKnownHubNames()` can answer alone, since both are explicitly node-local. `IClusterInventory` is a Core-level abstraction substituted exactly like every other Phase 3 interface — `LocalClusterInventory` (Registry, single-node: the node *is* the cluster) and `OrleansClusterInventory` (Orleans) — which is what keeps `Management` free of an `Orleans` project reference and keeps the single-node deployment mode fully working. It needed three additions, all appends under the existing `[Alias]`/`[Id(n)]` rules: `INodeRegistryGrain.GetAllNodesAsync()`, a cluster-wide hub directory piggybacked onto the same grain's existing `NodeRegistryPublisherService` republish cadence (union of `IHubRegistry.GetAllHubs()` and `ILocalTransportRegistry.GetKnownHubNames()`, for the same reason §7's Slice 7 bug note gives), and `IHubGrain.GetStatsAsync()` (plain counts, so a health check doesn't transfer every connection id across the cluster just to call `.Count()` on it).

## 13. Observability (Phase 4)

**Metrics: BCL `Meter` in Core, OpenTelemetry only in the host.** `Keryhe.Switchboard.Core.SwitchboardMetrics` owns a single `System.Diagnostics.Metrics.Meter` and every instrument (counters, histograms, and two node-local `ObservableGauge`s) — verified that a bare `net10.0` classlib with zero package references compiles `Meter`/`Counter`/`Histogram`/`ObservableGauge` cleanly, so instrumentation needs no OpenTelemetry reference at all. Only `Keryhe.Switchboard.Server` wires an OTLP exporter on top, and only when `SwitchboardOptions.OtlpEndpoint` is configured — with no endpoint set, the `Meter` still exists and still records, so unit tests observe every instrument via `System.Diagnostics.Metrics.MeterListener` with no collector, no exporter, and no network anywhere.

**The two gauges (`client_connections.active`, `server_connections.active`) are node-local by construction, not by convention.** Their `ObservableGauge` callbacks are registered once, in `Program.cs`, as closures over `ILocalTransportRegistry`/`IHubRegistry` — not as `SwitchboardMetrics` constructor dependencies, because `IHubRegistry` lives in `Keryhe.Switchboard.Protocol`, which Core cannot reference. Each callback reads only this node's own in-memory state, tagged by `hub`; the node identity itself is a resource attribute applied once to every exported signal (`ConfigureResource` in `Program.cs`), not a per-measurement tag — an `ObservableGauge` callback that summed the cluster via grain I/O on every collection interval would repeat `/healthz`'s original mistake, forever, on every node.

**`signalr.message.latency` (the roadmap's originally-planned client→server round-trip histogram) is not observable from a proxy and was replaced.** Correlating an `Invocation` with its `Completion` requires reading `invocationId` out of the payload on the hot path — exactly what `HubMessageClassifier` refuses to do (its own doc comment: widening it into a general parser is a review failure) — and the correlation would be structurally wrong for the cases that matter most anyway (streaming produces many `StreamItem`s per invocation; a fan-out send has no invocation at all). `signalr.message.inbound_duration` (client frame read → written to the assigned server connection, tagged `cross_node`) and `signalr.message.outbound_duration` (envelope received from an app server → written to the target client transport's channel) measure the proxy's own honest contribution instead — the actual operational question an operator can act on. True end-to-end latency stays a client-side measurement.

**`signalr.envelopes.unrouted` and `signalr.pending_connections.active` were both re-scoped from their Phase 1 deferral.** `envelopes.unrouted` was deferred as "unrouted group/user envelopes" — a counter for a deliberate log-and-no-op that Phase 2 went on to implement properly, so a metric under that name today would count nothing; it now counts, by `reason`, the drops that are real: `unknown_connection`, `malformed_server_connection_ref`, `server_connection_gone`, `no_payload_for_protocol`, and `no_node_subscribed` (the exact Phase 3 Slice 7 failure mode — worth the whole metric on its own, since a counter on it turns the next occurrence into an alert instead of an investigation). `pending_connections.active` (a gauge) would have needed an Orleans-mode "enumerate every outstanding grain" primitive that Phase 3's `IPendingConnectionStore` design (§1, plan decision D19) deliberately never built; `pending_connections.created`/`.consumed`/`.expired` (counters, derivable in both deployment modes from the store's existing call sites) give strictly more information — they distinguish "still outstanding" from "expired" from "never arrived" — for strictly less machinery, with in-flight count computed as `created - consumed - expired` in the backend.

**Tracing: negotiate and client-connect spans are always-on; per-message spans are opt-in.** `Keryhe.Switchboard.Core.SwitchboardTracing` owns a single `ActivitySource`, the tracing equivalent of `SwitchboardMetrics` — BCL-only, no OpenTelemetry reference needed to create or record spans. A "negotiate" span wraps `DefaultNegotiationService.NegotiateAsync`; a "client_connect" span wraps just the connection-establish steps in `ClientConnectionLifecycle.RunAsync` (registration through the app server's `OpenConnection` acknowledgment), not the connection's entire, potentially hours-long lifetime. Both are cheap (one per connection) and tagged `hub`/`connectionId`/`node.id`. A "message_route" span exists in `DefaultMessageRouter.RouteClientMessageAsync` but is only ever created when `SwitchboardOptions.TraceMessageRouting` is `true` (default `false`) — the gate lives at the call site, not inside `SwitchboardTracing` itself, since a span per routed message at broadcast fan-out rates is the cardinality equivalent of doing grain I/O in `/healthz` on every collection interval. There is deliberately **no trace-context propagation into `ServerEnvelope`** in this phase — adding a `TraceParent` field would be a legal append to the wire contract, but the parent/child linkage it would buy is obtainable more cheaply by tagging both sides with `connectionId` and correlating in the backend; revisit only if a real operational need shows attribute-based correlation is insufficient.

**Structured logging, not string interpolation, throughout.** Every `ILogger` call site in the codebase already used named-placeholder message templates (`"...{ConnectionId}..."`) rather than string interpolation, verified by a capturing `ILogger` that inspects the raw structured `state` object rather than the rendered message — the defect a naive `$"...{connectionId}..."` call would introduce is invisible to a substring assertion on the output but produces no `ConnectionId` field for a backend to filter, group, or alert on. Structured logs export through the same OTLP pipeline as traces and metrics (`builder.Logging.AddOpenTelemetry(...)`) when `OtlpEndpoint` is configured, so all three signals land in one backend together.

## 14. HubLifetimeManager Coverage (Phase 5)

### Why this section exists

[§8](#8-connector--negotiate-interception) and [§11](#11-connector--inbound-dispatch-synthetic-client-connections) cover the Connector's two hard mechanisms — negotiate interception and inbound dispatch — but neither discusses `SwitchboardHubLifetimeManager<THub>` (the *outbound* half: every `Clients.*` call) against the actual member surface of the framework class it replaces, `HubLifetimeManager<THub>`. ADR-005's own "drop-in for `AddAzureSignalR()`" claim implicitly depends on that coverage being complete, and Phase 5 finding 2 (`plans/phase-5-compatibility-testing-and-benchmarking.md`) is exactly the gap this section closes: a real, undocumented behavioral difference from Azure SignalR Service's server-side SDK that 4 phases of construction-phase tests never exercised, because none of them called a hub method that invokes a client.

### The member surface

`HubLifetimeManager<THub>` (`Microsoft.AspNetCore.SignalR.Core`, verified against the installed net10.0 runtime assembly) has **13 abstract members** and **3 virtual members**. `SwitchboardHubLifetimeManager<THub>` overrides all 13 abstract members — `OnConnectedAsync`, `OnDisconnectedAsync`, `SendAllAsync`, `SendAllExceptAsync`, `SendConnectionAsync`, `SendConnectionsAsync`, `SendGroupAsync`, `SendGroupsAsync`, `SendGroupExceptAsync`, `SendUserAsync`, `SendUsersAsync`, `AddToGroupAsync`, `RemoveFromGroupAsync` — pinned by a reflection-based test (`HubLifetimeManagerCoverageTests.EveryAbstractMember_IsOverridden`) rather than left to a compiler error alone, since a compiler error says nothing about *why* a missing override matters.

The 3 virtual members are where Azure SignalR Service and Switchboard both hit the same wall, for the same structural reason:

| Member | Base class default | Switchboard's override |
|---|---|---|
| `InvokeConnectionAsync<T>` | `throw new NotImplementedException("<T> does not support client return values.")` | Overridden — throws `NotSupportedException` naming Switchboard, pointing at [ADR-005](07-adr/ADR-005-protocol-compatibility.md#what-is-not-in-scope-for-compatibility) and [01-overview.md § Non-Goals](01-overview.md#non-goals) |
| `SetConnectionResultAsync` | same as above | Overridden — same `NotSupportedException` |
| `TryGetReturnType` | returns `false` ("no pending client-results invocation") | **Not overridden, deliberately** — the base answer is already correct, since this Connector never creates a pending invocation via `InvokeConnectionAsync` in the first place |

### Client results: why it's a non-goal, not a bug (plan decision D32)

`Clients.Client(id).InvokeAsync<T>(methodName)` — SignalR *client results*, a first-class feature since .NET 8 — lets hub code ask a specific client a question and await its answer. Before this phase, calling it against a Switchboard-fronted hub threw the framework's bare, unhelpful default (finding 2): nothing in the codebase named Switchboard, and nothing in the documentation said this was expected.

Implementing it for real is not a small addition. The invoking app server and the target client's assigned server connection can be different processes entirely once a client can be assigned to any node's app-server pool (plan decision D18) — so the client's eventual `CompletionMessage` has to be routed back to the *specific app server instance* that made the call, not just any app server serving that hub. That needs a new correlated-completion path (plausibly a new `ServerEnvelope` field carrying the originating app server's identity, plus cross-node forwarding analogous to what the backplane already does for `AddToGroup`/`RemoveFromGroup`) — Phase-3-sized work, not a fix that belongs inside a validation phase.

The decision (D32): treat it exactly like stateful reconnect — document it as a non-goal ([ADR-005](07-adr/ADR-005-protocol-compatibility.md#what-is-not-in-scope-for-compatibility), [01-overview.md § Non-Goals](01-overview.md#non-goals)) and make the failure mode name the actual limitation instead of leaking a framework implementation detail. `ClientResultsTests.HubCallingClientResults_SurfacesASwitchboardSpecificError_NotABareNotImplementedException` pins this: the invoking client's own `InvokeAsync<T>` still throws (the hub method still fails — this isn't silently swallowed), but the `HubException` message names Switchboard rather than reading like an unrelated SignalR internals bug. Full support is recorded as a candidate future enhancement, not scoped into any phase yet.
